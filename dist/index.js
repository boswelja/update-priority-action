...
// CONCATENATED MODULE: ./src/determine-update-priority.js



async function run() {
  // Get version strings
  const oldVersionStr = (0,core.getInput)('old-version', { required: true });
  const newVersionStr = (0,core.getInput)('new-version', { required: true });

  // Get update priorities
  const lowPriority = (0,core.getInput)('low-priority-result');
  const medPriority = (0,core.getInput)('med-priority-result');
  const highPriority = (0,core.getInput)('high-priority-result');

  // Parse version strings
  (0,core.info)('Parsing versions');
  const oldVersion = (0,semver.parse)((0,semver.clean)(oldVersionStr));
  const newVersion = (0,semver.parse)((0,semver.clean)(newVersionStr));

  // Check versions are valid
  if (!oldVersion) {
    (0,core.setFailed)(`Invalid old-version supplied; ${oldVersionStr}`);
    return;
  }
  if (!newVersion) {
    (0,core.setFailed)(`Invalid new-version supplied; ${newVersionStr}`);
    return;
  }

  // Determine the update priority
  (0,core.info)('Determining priority');
  let updatePriority;
  ...
  if (!updatePriority) {
    (0,core.warning)('Versions are the same, no update required');
  }
  (0,core.setOutput)('update-priority', updatePriority);
}
...
/******/ 	return __nccwpck_require__(__nccwpck_require__.s = 4734);
/******/ })()
;